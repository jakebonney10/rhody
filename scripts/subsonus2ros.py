...
def main():
    ...
    bad_rows = df[df[numeric_fields].isnull().any(axis=1)]
    ...
    for _, row in df.iterrows():
        ...
        fix.status.status = NavSatStatus.STATUS_FIX  # generic fix
        fix.status.service = 0  # acoustic positioning so no GNSS
